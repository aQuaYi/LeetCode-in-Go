...
func mincostToHireWorkers(quality []int, wage []int, K int) float64 {
	...
	for _, w := range workers {
		 ...
		 *
		 * 思考一下，如果 workers[i][1] (i>K) 是最大的 quality
		 * 那么，qsum += workers[i][1] 后，立马就会 qsum -= workers[i][1]
		 * 也就是说，qsum 中没有算上 workers[i][1]
		 * 但是，还是会执行
		 ...
		 * cost = min(cost, qsum*workers[i][0])
		 * 实际上是
		 * cost = min(<=qsum*workers[i-1][0], qsum*workers[i][0])
		 * 因为，根据 w[0] 的单调性
		 * workers[i-1][0] <= workers[i][0]
		 * 所以，
		 * qsum*workers[i][0] 虽然是一个错误的组合，但是也注定了这个值不会是最小值
		 *
		 */
	}
